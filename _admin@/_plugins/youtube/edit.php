...
// update cron time
$update = mysql_query("UPDATE `website_croned` SET `date`='".time()."' WHERE `id`=3 AND `type`='youtubeEdit' ");
...
`website_youtube_videos`.`upload_status`='updating' AND  
...
#######################################################################################################
$update = mysql_query("UPDATE `website_youtube_videos` SET `croned_time`='".time()."', `upload_status`='uploaded' WHERE `id`='".(int)$rows['wyv_id']."' "); 
...
$putUrl = "http://gdata.youtube.com/feeds/api/users/mavne1987able/uploads/".$video_link."?client=Null"; 
